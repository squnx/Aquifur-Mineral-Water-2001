...
@AUTHURLS = ('www.aquifur.com','aquifur.com');

$TO       = ('ivansong@earthlink.net,hwipark@hotmail.com');

$SUBJECT  = 'Order Request';

$REDIRECT = 'http://www.aquifur.com/order/confirm.html';

$SORT_TYPE = 'field';

@SORT_FIELDS = ('Name','Phone','Fax','Email','Street','Suite','City','Zip');
...
sub reformat_form_data
{
	...
	foreach $pair (@pairs) 
	{
		...
		$value =~ s/<!--(.|\n)*-->//g;
		if ($name eq 'email' || $name eq 'LastName' && ($value)) 
		{
		      	$CONFIG{$name} = $value;
		}
...
# Build the 'from' address of the form: "name <email address>"
	...
	$from_name=($CONFIG{'LastName'} . " <" . $CONFIG{'email'} . "> ");
...
# Output the mail message header with the Local Date/Time
	...
	print MAIL "   $CONFIG{'LastName'} on $date\n\n";
	print MAIL "---------------------------------------------------------------\n\n";
